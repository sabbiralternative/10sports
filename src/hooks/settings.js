  ...
  return useMutation({
    ...
    mutationFn: async () => {
      ...
      if (data?.success) {
        ...
        if (!isLocalhost) {
          const logo = `${API.assets}/${Settings.site_name}/logo.${Settings.logo_format}`;
          setLogo(logo);
        } else {
        ...
        if (!isLocalhost) {
          link.href = `${API.assets}/${Settings.site_name}/theme.css`;
        ...
        FavIconLink.href = `${API.assets}/${Settings.site_name}/favicon.png`;
